...
async function run() {
    try {
        ...
        const studentCollection = client.db('admissionTo').collection('student');
        ...
        app.put("/student/:id", async (req, res) => {
            ...
            const result = await studentCollection.updateMany(filter, updateDoc, options);
            res.send(result);
          });
...
app.get('/', (req, res) => {
    res.send('Admission College');
});

app.listen(port, () => {
    console.log(`Admission College is start on port ${port}`);
});
